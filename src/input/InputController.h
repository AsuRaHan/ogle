...
namespace ogle {
	...
	// Регистрация оси
		void RegisterAxis(const std::string& name, const std::vector<AxisBinding>& bindings);
		void RegisterAxis(const std::string& name, const AxisBinding& binding);

		// Получение значения оси
		float GetAxis(const std::string& name) const;
		float GetAxisRaw(const std::string& name) const;  // Без smoothing

		// Обновление осей (вызывать каждый кадр)
	...
	private:
		...
		struct AxisData {
			...
			float value = 0.0f;           // Текущее значение (с smoothing)
			float rawValue = 0.0f;        // Сырое значение
			float targetValue = 0.0f;     // Целевое значение
		};

		std::unordered_map<std::string, AxisData> m_axes;

		// Методы для вычисления значений
		float CalculateAxisValue(const AxisData& axisData) const;
		void UpdateAxis(const std::string& name, AxisData& axisData, float deltaTime);
