...
namespace ogle {
	...
	// Вспомогательные функции (переносим реализацию вверх)
	...
	void InputSystem::Update(float deltaTime) {
		...
		// Здесь позже добавим UpdateActions
	...
	bool InputSystem::IsKeyPressed(int vk) const {
		return m_keyPressedThisFrame[vk];
	}

	bool InputSystem::IsKeyReleased(int vk) const {
	...
	bool InputSystem::IsMouseButtonDown(int button) const {
		return button >= 0 && button < 5 && m_mouseButtonStates[button];
	}

	bool InputSystem::IsMouseButtonPressed(int button) const {
		return button >= 0 && button < 5 && m_mousePressedThisFrame[button];
	}

	bool InputSystem::IsMouseButtonReleased(int button) const {
		return button >= 0 && button < 5 && m_mouseReleasedThisFrame[button];
	}

	bool InputSystem::IsGamepadConnected(int player) const {
		return player >= 0 && player < 4 && m_gamepads[player].connected;
	}

	bool InputSystem::IsGamepadButtonDown(int player, int button) const {
		return player >= 0 && player < 4 && button >= 0 && button < 14 &&
			m_gamepads[player].connected && m_gamepads[player].buttons[button];
	}

	float InputSystem::GetGamepadAxis(int player, int axis) const {
		if (player >= 0 && player < 4 && axis >= 0 && axis < 6 &&
			m_gamepads[player].connected) {
	...
	InputAction* InputSystem::CreateAction(const std::string& name, ActionType type) {
		// Добавьте реализацию вместо заглушки
	...
	void InputSystem::RemoveAction(const std::string& name) {
		if (m_actions.erase(name) > 0) {
			Logger::Debug("InputAction removed: " + name);
		}
	}


	// ==================== Контексты ====================
	void InputSystem::PushContext(const std::string& context) {
	...
	const std::string& InputSystem::GetCurrentContext() const {
